...
#[test]
fn includes_and_excludes_static_unions_in_wire_order() {
    let source = Segmented;
    let selected = root(&source).include(|f| f.signature | f.header.checksum | f.header);
    assert_eq!(selected.byte_len(), 5);
    assert_eq!(bytes(selected), [0, 1, 2, 7, 8]);

    let excluded = root(&source).exclude(|f| f.signature | f.header.checksum | f.header);
    assert_eq!(excluded.byte_len(), 7);
    assert_eq!(bytes(excluded), [0xa5, 0xa5, 0xa5, 0xa5, 0xb6, 0xb6, 11]);
}
...
#[test]
fn markers_and_recursive_unions_do_not_store_schema_descriptors() {
    assert_eq!(size_of::<Fields>(), 0);
    assert_eq!(size_of::<Payload>(), 0);
    assert_eq!(size_of::<FieldUnion<Header, Checksum>>(), 0);
    assert_eq!(
        size_of::<FieldUnion<FieldUnion<Header, Checksum>, Signature>>(),
        0
    );
    assert_eq!(
        size_of::<ByteSelection<'_, Segmented, Fields>>(),
        size_of::<&Segmented>()
    );
    assert_eq!(
        size_of::<wire_repr::IncludedBytes<'_, Segmented, Payload>>(),
        size_of::<&Segmented>() + size_of::<usize>()
    );
}
