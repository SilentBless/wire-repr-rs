//! Owned `bytes` dynamic struct encoding rendering.
...
pub(super) struct Fragments {
    ...
    pub(super) builder_method: TokenStream,
...
pub(super) fn render(input: &Input<'_>, shape: &TypeShape) -> Fragments {
    ...
    if operation_input_ty.is_some() {
        ...
        Fragments {
            ...
            builder_method: quote!(#builder_method),
            inherent_methods: quote! {
                #vis fn #operation_name(self, operation: &#operation_input_ty) -> #encode_request_type {
                    #encode_request {
                        value: self,
                        operation,
                    }
