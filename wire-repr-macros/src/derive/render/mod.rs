...
/// Returns the direct nested wire type path instantiated for generated view backing.
fn generated_view_path(ty: &syn::Type) -> syn::Result<TokenStream> {
    ...
    let mut type_path: syn::TypePath = syn::parse2(quote::quote!(#type_path))?;
    let lifetime = if cfg!(feature = "bytes") {
        syn::Lifetime::new("'static", Span::call_site())
    } else {
        syn::Lifetime::new("'__wire_repr_wire", Span::call_site())
    };
    if let Some(segment) = type_path.path.segments.last_mut()
        && let syn::PathArguments::AngleBracketed(arguments) = &mut segment.arguments
    {
        for argument in &mut arguments.args {
            if let syn::GenericArgument::Lifetime(argument_lifetime) = argument {
                *argument_lifetime = lifetime.clone();
            }
        }
    }
    Ok(quote::quote!(#type_path))
}
...
fn generated_decode_error_path(ty: &syn::Type, runtime: &TokenStream) -> syn::Result<TokenStream> {
    let view = generated_view_path(ty)?;
    let lifetime = if cfg!(feature = "bytes") {
        quote::quote!('static)
    } else {
        quote::quote!('__wire_repr_wire)
    };
    Ok(quote::quote!(
        <#view as #runtime::WireViewType>::DecodeError<#lifetime>
    ))
}
