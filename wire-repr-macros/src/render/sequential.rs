//! Fixed-sequential Rust token rendering.
...
pub(super) fn render_layout(layout: &SequentialLayout) -> TokenStream {
    ...
    let width = offset_expression(layout, layout.physical_order.len());
    let zero_width: Vec<_> = zero_width_checks(layout, error_name).collect();
    let parse_extent_checks: Vec<_> = extent_checks(layout, error_name).collect();
    let getters = data.fields.iter().flat_map(|field| {
        let mut methods = render_getter(layout, field).into_iter().collect::<Vec<_>>();
    ...
    let mutation_display_arms = encode_display_arms(data.fields.iter());
    let setters = data
        .fields
        .iter()
    ...
    let write_display_arms = encode_display_arms(data.fields.iter());
    let missing_checks = data.fields.iter().filter_map(|field| {
        field.codec()?;
        let name = &field.name;
    ...
    let builder_zero_width: Vec<_> = builder_zero_width_checks(layout, write_error_name).collect();
    let write_extent_checks: Vec<_> = extent_checks(layout, write_error_name).collect();
    let builder_preflight: Vec<_> =
        builder_preflight(data.fields.iter(), write_error_name).collect();
    ...
    let plan_names: Vec<_> = data
        ...
        .collect();
    let commits = layout.physical_order.iter().filter_map(|item| {
        ...
        Some(quote! {
            ::wire_repr::EncodePlan::write_into(
                &#plan,
                &mut bytes[(#start)..((#start) + <#codec as ::wire_repr::FixedCodec>::WIDTH)],
            );
        })
    });
    ...
    quote! {
        ...
        #[doc = "An immutable byte-backed view of a fixed sequential wire layout."]
        #(#docs)*
        ...
        #[derive(Debug)]
        #[doc = "Reports why parsing this fixed sequential layout failed."]
        #visibility enum #error_name {
            ...
            InvalidLayoutExtent { #[doc = "The one-based physical position of the overflowing item."] position: usize, #[doc = "The represented offset before this item in bytes."] offset: usize, #[doc = "The byte advance requested by this item."] advance: usize },
        }
        ...
        impl<'wire> #layout_name<'wire> {
            #[doc = "The saturated encoded width of this layout in bytes."]
            #visibility const WIDTH: usize = #width;
            ...
            #[doc = "Returns the exact bytes represented by this view."]
            #[must_use]
        ...
        #[doc = "A mutable byte-backed view of a fixed sequential wire layout."]
        #(#docs)*
        ...
        impl<'wire> #view_mut_name<'wire> {
            ...
            #[doc = "Returns the exact bytes represented by this mutable view."]
            #[must_use]
        ...
        #[derive(Debug)]
        #[doc = "Reports why a fixed sequential field mutation could not be prepared."]
        ...
        #[doc = "An atomic fluent builder for a fixed sequential wire layout."]
        #(#docs)*
        #visibility struct #builder_name<'value> { #(#builder_fields,)* }

        #[doc = "A prepared fixed sequential layout encoding."]
        ...
        #[derive(Debug)]
        #[doc = "Reports why an atomic fixed sequential builder write could not complete."]
        #visibility enum #write_error_name {
            ...
            #(#write_variants)*
            #[doc = "Reports a fixed codec that declares zero width."]
            InvalidCodecWidth { #[doc = "The field whose codec declared zero width."] field: &'static str },
            #[doc = "Reports a successful codec plan with an invalid encoded length."]
            InvalidPlanLength { #[doc = "The field whose plan was invalid."] field: &'static str, #[doc = "The codec width required for the field."] expected: usize, #[doc = "The length reported by the plan."] actual: usize },
            #[doc = "Reports a physical layout extent that cannot be represented in `usize`."]
            InvalidLayoutExtent { #[doc = "The one-based physical position of the overflowing item."] position: usize, #[doc = "The represented offset before this item in bytes."] offset: usize, #[doc = "The byte advance requested by this item."] advance: usize },
            #[doc = "Reports output shorter than the complete layout."]
        ...
        impl ::core::fmt::Display for #write_error_name { fn fmt(&self, formatter: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result { match self {
            ...
            #(#write_display_arms)*
            Self::InvalidCodecWidth { field } => write!(formatter, "field {field} has a fixed codec with zero width"),
            Self::InvalidPlanLength { field, expected, actual } => write!(formatter, "field {field} plan length: expected {expected} bytes, got {actual}"),
            Self::InvalidLayoutExtent { position, offset, advance } => write!(formatter, "layout extent at position {position} overflows: offset {offset} plus advance {advance}"),
            Self::OutputTooShort { needed, available } => write!(formatter, "output too short: need {needed} bytes, got {available}"),
        } } }
...
fn encode_display_arms<'a>(
    ...
    fields.filter_map(|field| { field.codec()?; let variant=&field.error_variant; let name=field.name.to_string(); Some(quote!(Self::#variant(error) => write!(formatter, "field {} failed encoding: {error:?}", #name),)) })
}
fn zero_width_checks<'a>(
    layout: &'a SequentialLayout,
    error_name: &'a syn::Ident,
) -> impl Iterator<Item = TokenStream> + 'a {
    layout.physical_order.iter().filter_map(move |item| { let PhysicalItem::Field { index, position } = item else { return None; }; let codec=codec_tokens(layout.data.fields[*index].codec()?); Some(quote!(if <#codec as ::wire_repr::FixedCodec>::WIDTH == 0 { return Err(#error_name::InvalidCodecWidth { position: #position }); })) })
}
fn builder_zero_width_checks<'a>(
    layout: &'a SequentialLayout,
    error_name: &'a syn::Ident,
) -> impl Iterator<Item = TokenStream> + 'a {
    layout.physical_order.iter().filter_map(move |item| {
        let PhysicalItem::Field { index, .. } = item else {
            return None;
        };
        ...
        Some(quote! {
            if <#codec as ::wire_repr::FixedCodec>::WIDTH == 0 {
                return Err(#error_name::InvalidCodecWidth { field: #field_text });
            }
        })
    })
}
...
fn field_offset(layout: &SequentialLayout, field_index: usize) -> TokenStream {
    offset_expression(layout, layout.data.fields[field_index].placement - 1)
}
fn builder_preflight<'a>(
    fields: impl Iterator<Item = &'a Field> + 'a,
    error_name: &'a syn::Ident,
) -> impl Iterator<Item = TokenStream> + 'a {
...
fn extent_checks<'a>(
    ...
    ::core::iter::once(quote!(let mut __wire_offset = 0usize;)).chain(
        layout.physical_order.iter().filter_map(move |item| {
            let (position, advance) = match item {
                PhysicalItem::Field { index, position } => {
                    let codec = codec_tokens(layout.data.fields[*index].codec()?);
                    (
                        *position,
                        quote!(<#codec as ::wire_repr::FixedCodec>::WIDTH),
                    )
                }
                PhysicalItem::Padding { length, position } => (*position, quote!(#length)),
                PhysicalItem::Alignment {
                    boundary: 1,
                    position,
                } => (*position, quote!(0usize)),
                PhysicalItem::Alignment { boundary, position } => (
                    *position,
            ...
            Some(quote! {
                ...
                __wire_offset = match __wire_offset.checked_add(__wire_advance) {
                    ::core::option::Option::Some(next) => next,
                    ::core::option::Option::None => return Err(#error_name::InvalidLayoutExtent {
                        position: #position,
                        offset: __wire_offset,
                        advance: __wire_advance,
                    }),
                };
            })
        }),
...
fn offset_expression(layout: &SequentialLayout, count: usize) -> TokenStream {
    layout.physical_order[..count]
        .iter()
        .fold(quote!(0usize), |offset, item| {
            advance_offset(layout, offset, item)
        })
}
fn advance_offset(
    layout: &SequentialLayout,
    ...
    match item {
        PhysicalItem::Field { index, .. } => match layout.data.fields[*index].codec() {
            Some(codec) => {
                ...
                quote!((#offset).saturating_add(<#codec as ::wire_repr::FixedCodec>::WIDTH))
            }
            None => offset,
        },
        PhysicalItem::Padding { length, .. } => quote!((#offset).saturating_add(#length)),
        PhysicalItem::Alignment { boundary: 1, .. } => offset,
        PhysicalItem::Alignment { boundary, .. } => {
            quote!({ let offset = #offset; offset.saturating_add((#boundary - (offset % #boundary)) % #boundary) })
        }
    }
}
