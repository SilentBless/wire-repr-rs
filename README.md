...
For example, IPv4 IHL counts 32-bit words, so parsing converts `5` to 20 bytes while
builder preparation converts an aligned 20-byte header back to the canonical value `5`.
...
```rust
wire_repr! {
    pub layout WordCountFrame {
        body_words: U8 { range_source: crate::FourByteWords; };
        body: bytes(body_words);
    }
...
plan so commit remains capacity-only and atomic. This owns structural arithmetic, not
protocol policy: rules such as minimum IPv4 IHL or UDP length still belong to the
consumer.
